...
/**
 * Loads PEC Table 4.30.14.5(A) (single-phase locked-rotor current, for selecting
 * disconnecting means and controllers) from the classpath once, then serves per-size,
 * per-voltage lookups.
 *
...
public class LockedRotorSinglePhaseTable {
	...
	public LockedRotorSinglePhaseTable() {
		this.entries = loadEntries();
	}

	private static Map<String, LockedRotorEntry> loadEntries() {
		ObjectMapper objectMapper = new ObjectMapper();
		try (InputStream in = LockedRotorSinglePhaseTable.class.getResourceAsStream(RESOURCE_PATH)) {
			if (in == null) {
	...
	public double lookup(String sizeLabel, int voltage) {
		...
		if (entry == null) {
			throw new CalculationException(
					"No single-phase locked-rotor data for size " + sizeLabel + ", voltage " + voltage + "V");
		}
